...
for t in {1..10..1}; do 
  ...
  for size in {1..18..1}; do 
    let num_partlen=$total_partlen_elems/$num_tasks
     mpirun --map-by ppr:1:node --host $FLAGS  $hosts -x LD_PRELOAD=/home/projects/x86-64/gcc/10.2.0/lib64/libstdc++.so.6 -x OMP_PLACES=cores -x OMP_NUM_THREADS=$num_threads -x QTHREAD_STACK_SIZE=8196 -x OMP_PROC_BIND=true $binary $num_tasks $num_threads $num_part $num_partlen
